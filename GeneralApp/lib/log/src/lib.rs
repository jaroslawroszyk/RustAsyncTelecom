...
fn clear_logs(path: &String) -> anyhow::Result<()> {
...
pub fn log(level: usize, loc: &Location, content: String) {
    ...
    match level {
        ...
        4 => log::debug!("({thread}) [{loc}] {content}"),
        5 => log::trace!("({thread}) [{loc}] {content}"),
        _ => (),
    }
}
*/
...
pub fn log(level: usize, loc: &Location, content: String) {
    ...
    // println!("JAREK file_path: {}", file_path);
    ...
    match level {
        ...
        2 => log::warn!("({thread}) {content}"),
        3 => log::info!("({thread}) {content}"),
        // 4 => log::debug!("({thread}) [{module_name}] {}{} {content}", loc.column(),),
        4 => log::debug!(
